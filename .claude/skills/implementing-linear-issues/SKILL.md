...
5. Run the relevant build/test commands and confirm they pass before proceeding: `dotnet build` and `dotnet test` always; for a Postgres change, `docker compose up -d` and `./db/migrate.sh` first so the change can actually be exercised locally.
...
- **`writing-fsharp-code`** / **`writing-postgresql-code`** — whichever the Issue names; invoked from step 4 to do the actual implementation, including its own approval gate.
