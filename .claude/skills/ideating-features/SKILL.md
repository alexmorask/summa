...
3. Generate candidate ideas at product level only: the problem or opportunity, why it fits the current roadmap ordering, which bounded context(s) it touches. No schema, code, or infra design — that's out of scope for this agent entirely. When weighing trade-offs across multiple candidate ideas, reach for `sequentialthinking` to work through them explicitly rather than converging on the first plausible one.
