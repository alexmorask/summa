...
description: This skill should be used by tech-lead once a Linear Project is feasible and needs to be broken into Issues — for example "break this into issues", "plan the work for this Project", or automatically after assessing-feasibility returns a Feasible verdict. For infra-only work with no product-manager handoff, first invokes drafting-linear-projects to originate the Project. Discovers available writing-* skills live and creates exactly one Issue per skill invocation, per the recorded Issue-granularity decision. Always shows the full set of draft Issues and waits for explicit owner go-ahead before writing any of them to Linear.
---

## Purpose

Turn a feasible Linear Project into concrete, buildable Issues — one per `writing-*` skill invocation — always shown as a draft set and written only after explicit owner approval.
...
2. Discover the available `writing-*` skills live via glob (`.claude/skills/writing-*/SKILL.md`) — never a hardcoded list; this set grows as the repo does.
3. Break the Project's scope into discrete units of work, each mapped to exactly one `writing-*` skill invocation — the recorded 2026-08-12 Issue-granularity decision (one Issue per skill invocation, not one per Project or one per file).
4. Draft each Issue: title, description, which `writing-*` skill it maps to, and team. Show the complete set of draft Issues together and wait for the owner's explicit go-ahead before calling any Linear write tool.
